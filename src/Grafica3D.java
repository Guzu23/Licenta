...
public class Grafica3D extends Frame implements GLEventListener {
		...
		window.addMouseListener(new MouseAdapter() {
			...
			public void mouseWheelMoved(MouseEvent e) {
				...
				if (notches < 0) {
					...
					if (camera.PLAYER_SPEED < 0)
						camera.PLAYER_SPEED = 0.0f;
				} else {
	...
	void updateCameraView(int deltaX, int deltaY) {
		// Update camera yaw and pitch angles based on mouse movement
		float yawDelta = (float) Math.toRadians(-deltaX * camera.MOUSE_SENSITIVITY);
		float pitchDelta = (float) Math.toRadians(-deltaY * camera.MOUSE_SENSITIVITY);
		...
		if (camera.pitch < camera.PITCH_MAX && camera.pitch > camera.PITCH_MIN) {
			horizontalRotation.rotate(-yawDelta, 0.0f, (float) (Math.cos(camera.pitch)),
					(float) (-Math.sin(camera.pitch)));
			horizontalRotation.rotate(-pitchDelta, 1.0f, 0.0f, 0.0f);
		} else
			horizontalRotation.rotate((float) (-yawDelta), 0.0f, 0.0f, (float) (-Math.sin(camera.pitch)));
	...
	void updateCameraPosition() {
		...
		if (d_pressed)
			translateCamera(-cos_yaw_times_player_speed, 0, sin_yaw_times_player_speed);
		if (space_pressed)
			translateCamera(0, -camera.PLAYER_SPEED, 0);
		if (shift_pressed)
			translateCamera(0, camera.PLAYER_SPEED, 0);
	}
	...
	void update() {
		...
		// updateCameraView(); already did by the mouse listener
	}
